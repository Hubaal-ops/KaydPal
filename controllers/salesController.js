...
async function insertSale(sale) {
  ...
  // Update customer balance and account using Mongoose
...
async function updateSale(sel_no, updatedSale, userId) {
  ...
  if (nQty > oldSale.qty) {
    console.log('Checking stock for update:', { product_no: Number(product_no), store_no: Number(store_no) });
    const stock = await StoreProduct.findOne({ product_no: Number(product_no), store_no: Number(store_no) });
  ...
  // Reverse old sale effects using StoreProduct (Mongoose)
  await StoreProduct.updateOne(
  ...
  // Reverse old customer and account effects using Mongoose
  await Customer.updateOne(
  ...
  // Apply new sale effects using StoreProduct (Mongoose)
  ...
  // Apply new customer and account effects using Mongoose
  if (newDebt > 0) {
    await Customer.updateOne(
      { customer_no: Number(customer_no) },
      { $inc: { bal: newDebt } }
...
async function deleteSale(sel_no, userId) {
  try {
    ...
    // Reverse inventory in StoreProduct using Mongoose
    await StoreProduct.updateOne(
      { product_no: Number(oldSale.product_no), store_no: Number(oldSale.store_no) },
      { $inc: { qty: oldSale.qty }, $set: { updated_at: new Date() } }
    );
    ...
    // Reverse customer and account effects using Mongoose
    if (unpaid > 0) {
      await Customer.updateOne(
        { customer_no: Number(oldSale.customer_no) },
        { $inc: { bal: -unpaid } }
...
async function getAllSales(req, res) {
  try {
    ...
    // Fetch all related data using Mongoose
    ...
    const [products, customers, stores, accounts] = await Promise.all([
      Product.find({ product_no: { $in: productNos } }).lean(),
      Customer.find({ customer_no: { $in: customerNos } }).lean(),
      Store.find({ store_no: { $in: storeNos } }).lean(),
      Account.find({ account_id: { $in: accountIds } }).lean(),
    ]);
